...
ALLOW_DOMAINS = ["linkedin", "linkedin.com", "greenhouse.io", "lever.co", "workday", "smartrecruiters"]
...
def extract_text(url):
    ...
    return text[:50000]  # keep reasonable length
...
def main():
    for name in os.listdir(JOBS_DIR):
        ...
        if os.path.exists(out_path):
            continue
        all_texts = []
        for u in links[:2]:  # try first two links
            try:
                txt = extract_text(u)
